...
-   '.' -- is a wildcard for match of everthing for any single that similarly works as the '?' metacharacter.
        -   To specifically match a period or a '.' character, you have to place a '\' before the '.' character.  This mean a '.' of the input can be evaluaated or match by the \. only.  The is why in our regex, '/^([a-z0-9_\.-]+)@([\da-z\.-]+)__\.__([a-z\.]{2,6})$/ we have a \. notation in between 
